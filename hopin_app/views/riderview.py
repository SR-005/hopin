...
def riderpoll(request):
    ...
    stalerequestids = list(userrequests.filter(
        trip_exists=False).values_list("id", flat=True))
    if stalerequestids:
...
def rebookable(pastrides):
    preferredtrips = []
    driveremails = list(pastrides.values_list(
        'trip__usercredentials__email', flat=True).distinct())
    print("Drivers: ", driveremails)

    for driveremail in driveremails:
        driverobject = User.objects.get(email=driveremail)
        activeagain = trip.objects.filter(
            usercredentials=driverobject, status__in=["EMPTY", "ACTIVE"])
        if activeagain.exists():
            preferredtrips.append(activeagain.first())
    return preferredtrips
...
def requesttimevalidation(request):
    ...
    ridetime = datetime.strptime(request.POST.get("ridetime"), "%H:%M").time()
    ridedate = datetime.strptime(
        request.POST.get("ridedate"), "%Y-%m-%d").date()
    ...
    if ridedate not in [today, today + timedelta(days=1)]:
        messages.error(
            request, "Rides can only be Requested for Today or Tomorrow")
        return redirect("rider")

    if direction == "to":
        if ridedate == today and now > time(8, 0):
            messages.error(
                request, "Cannot schedule today's 8:00 AM ride after it has passed")
            return redirect("rider")
    ...
    # time validation: FROM college (between 1:30PM and 8:00PM)
        ...
        if ridetime < time(13, 30) or ridetime > time(20, 00):
            messages.error(
                request, "Rides can only be requested between 1:30PM and 8:00PM")
            return redirect("rider")
...
def riderdetails(request):
    ...
    availabletrips = []
    if direction == "to":
        activetrips = trip.objects.filter(
            prefereddirection=direction, ridedate=date, status__in=["ACTIVE", "EMPTY"],)
    else:
        activetrips = trip.objects.filter(prefereddirection=direction, ridedate=date, ridetime__range=(lcutofftime, ucutofftime),
                                          status__in=["ACTIVE", "EMPTY"])
    for trips in activetrips:
...
def requestride(request):
    if riderequest.objects.filter(rider=request.user, status="ACCEPTED").exists():
        messages.error(
            request, "You already have an Accepted Ride. Cannot send more Requests")
        return redirect("rider")
    ...
    pickupindex, _ = routesegmentation(
        (float(latitude), float(longitude)), ride.routegeometry)
    amount = requestprice(
        ride, pickupindex, (float(latitude), float(longitude)))
...
def requestrideagain(request, pastrides):
    ...
    pastride = pastrides.filter(
        trip__usercredentials=currenttrip.usercredentials).first()
    location = (float(pastride.pickuplatitude),
                float(pastride.pickuplongitude))
...
def cancelrequest(request):
    ...
    if timeremaining <= timedelta(minutes=30):
        messages.error(
            request, "Cannot cancel a ride within 30 minutes of start time.")
        return redirect("rider")

    if cancelrequest.status == "ACCEPTED":
        print(cancelride.availableseats)
        # increase trip seats if ride is cancelled after acceptance
        cancelride.availableseats = cancelride.availableseats+1
        cancelride.save()

    cancelrequest.delete()

    acceptedrequests = riderequest.objects.filter(
        trip=cancelride, status="ACCEPTED")
    print("AR: ", len(acceptedrequests))
...
def seemore(request):
    ...
    if ride.status != "ONGOING":
        ...
        return redirect("tracking", rideid=rideid)
